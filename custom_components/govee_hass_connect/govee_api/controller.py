...
# Fingerprint prefix used for statically-configured devices whose real MAC
# address (device identifier) is not yet known.
_STATIC_FINGERPRINT_PREFIX = "static:"


@dataclass
class GoveeDeviceInfo:
    """Configuration for a device added by direct IP (e.g. across VLAN).

    When ``sku`` is provided the device is registered immediately so it can be
    controlled before a scan response is received.  The real fingerprint and
    capabilities are reconciled transparently when/if the device responds to a
    unicast scan.

    Args:
        ip: IPv4 address of the device.
        sku: Optional Govee model identifier (e.g. ``"H6001"``).  When
            supplied the capability set is looked up from the built-in
            registry and the device object is created immediately.
        fingerprint: Optional MAC-style identifier.  When omitted and *sku*
            is given a placeholder fingerprint is used until the device
            responds to a scan.
    """
...
class GoveeController(asyncio.DatagramProtocol):
    def __init__(
        ...
        static_devices: list[str | GoveeDeviceInfo] | None = None,
        ...
        Args:
            ...
            static_devices: Optional list of device IPs or :class:`GoveeDeviceInfo` objects to register
                directly, without relying on multicast discovery.  Useful when the Home Assistant host
                and Govee devices are on different network segments (VLANs) where multicast does not
                cross.  Each entry can be a plain IP string or a :class:`GoveeDeviceInfo` with an
                optional SKU and fingerprint for immediate capability resolution.
        """
        ...
        # Tracks IPs that were explicitly configured as static — these are
        # never evicted and always receive periodic scan/update messages.
        self._static_device_ips: set[str] = set()

        # Normalise the static_devices constructor argument into GoveeDeviceInfo
        # objects so start() can process them after the transport is ready.
        self._pending_static_devices: list[GoveeDeviceInfo] = []
        for entry in static_devices or []:
            if isinstance(entry, str):
                self._pending_static_devices.append(GoveeDeviceInfo(ip=entry))
            else:
                self._pending_static_devices.append(entry)
    ...
    async def start(self):
        ...
        for info in self._pending_static_devices:
            self._register_static_device(info)

        needs_discovery = (
            self._discovery_enabled
            or self._registry.has_queued_devices
            or bool(self._static_device_ips)
        )
        if needs_discovery:
    ...
    def add_device_by_ip(
        ...
        *,
        ...
        """Register a device by direct IP address.

        Suitable for segmented-network (VLAN) deployments where multicast
        discovery does not cross network boundaries.  Unicast scan messages
        are sent to the device on every discovery interval so fingerprint and
        capabilities are refined automatically once the device responds.

        If *sku* is provided the device is immediately available for control
        commands even before it answers a scan.  Otherwise the device object
        is created when the first scan response arrives.

        Args:
            ip: IPv4 address of the Govee device.
            sku: Optional model identifier (e.g. ``"H6001"``).
            fingerprint: Optional MAC-style device identifier.

        Returns:
            The :class:`GoveeDevice` instance when *sku* is provided (device
            pre-created), or ``None`` when waiting for the first scan response.
        """
        info = GoveeDeviceInfo(ip=ip, sku=sku, fingerprint=fingerprint)
        device = self._register_static_device(info)
        if self._transport and not self._discovery_enabled:
            self.send_discovery_message()
        return device

    def _register_static_device(self, info: GoveeDeviceInfo) -> GoveeDevice | None:
        """Internal: register a static device config after the transport is up.

        Returns the pre-created GoveeDevice when SKU is known, else None
        (device will be created on first scan response).
        """
        ip = info.ip
        self._static_device_ips.add(ip)

        if info.sku:
            # Check for pre-existing device at this IP to avoid duplicates
            if existing := self._registry.get_device_by_ip(ip):
                self._logger.debug(
                    "Static device already registered at %s (%s)",
                    ip,
                    existing.fingerprint,
                )
                return existing

            fingerprint = info.fingerprint or f"{_STATIC_FINGERPRINT_PREFIX}{ip}"
            sku = info.sku
            capabilities = GOVEE_LIGHT_CAPABILITIES.get(sku) or ON_OFF_CAPABILITIES
            if sku not in GOVEE_LIGHT_CAPABILITIES:
                self._logger.warning(
                    "Static device %s sku=%s is not in the capability registry; "
                    "only power control will be available.",
                    ip,
                    sku,
                )
            device = GoveeDevice(self, ip, fingerprint, sku, capabilities)
            device.is_manual = True
            device = self._registry.add_discovered_device(device)
            self._logger.info(
                "Static device registered: ip=%s sku=%s fingerprint=%s",
                ip,
                sku,
                fingerprint,
            )
            self._call_discovered_callback(device, True)
            return device
        else:
            # SKU unknown — queue IP for unicast scan; device created on response
            added = self._registry.add_device_to_queue(ip)
            if added:
                self._logger.info(
                    "Static device IP queued for unicast discovery: %s", ip
                )
            return None
    ...
    async def _handle_scan_response(self, message: ScanResponse) -> None:
        ...
        if device := self.get_device_by_fingerprint(fingerprint):
            ...
            # Reconcile: a static device may have been pre-created with a
            # placeholder fingerprint.  If so, update it in place rather than
            # creating a duplicate entry.
            static_placeholder = self._registry.get_device_by_ip(ip)
            if (
                static_placeholder is not None
                and static_placeholder.fingerprint.startswith(
                    _STATIC_FINGERPRINT_PREFIX
                )
            ):
                resolved_sku = sku or static_placeholder.sku
                self._logger.info(
                    "Resolved static device at %s: fingerprint=%s sku=%s",
                    ip,
                    fingerprint,
                    resolved_sku,
                )
                self._registry.update_device_fingerprint(
                    static_placeholder, fingerprint
                )
                if sku and sku != static_placeholder.sku:
                    static_placeholder.update_sku(sku)
                    capabilities = (
                        GOVEE_LIGHT_CAPABILITIES.get(sku) or ON_OFF_CAPABILITIES
                    )
                    static_placeholder.update_capabilities(capabilities)
                static_placeholder.update_lastseen()
                self._call_discovered_callback(static_placeholder, False)
            else:
                if not sku:
                    self._logger.warning(
                    ...
                    if sku not in GOVEE_LIGHT_CAPABILITIES:
                        self._logger.warning(
                            "Device %s is not supported. Only power control is available. "
                            "Please open an issue at 'https://github.com/Galorhallen/govee-local-api/issues'",
                            sku,
                        )

                device = GoveeDevice(
                    self, ip, fingerprint, sku or "UNKNOWN", capabilities
                )
    ...
    def _evict(self) -> None:
        ...
        for fingerprint, device in devices.items():
            # Static/manually-configured devices are never evicted; they remain
            # registered so unicast control continues to work even when the
            # device has been unreachable for a while.
            if device.ip in self._static_device_ips:
                continue
