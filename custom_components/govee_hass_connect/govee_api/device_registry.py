...
class DeviceRegistry:
    ...
    def update_device_fingerprint(
        self, device: GoveeDevice, new_fingerprint: str
    ) -> None:
        """Replace the registry key for a device with a new fingerprint."""
        old_fingerprint = device.fingerprint
        if old_fingerprint in self._discovered_devices:
            del self._discovered_devices[old_fingerprint]
        device.update_fingerprint(new_fingerprint)
        self._discovered_devices[new_fingerprint] = device
